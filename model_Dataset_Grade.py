...
checkpointer = ModelCheckpoint(
    filepath=os.path.join('Data', 'checkpoints', 'model_Dataset_Grade_2.hdf5'),
    verbose=1,
    save_best_only=True)
...
timestamp = time.time()
csv_logger = CSVLogger(os.path.join('Data', 'logs', 'model_Dataset_Grade_2' + '-' + 'training-' + \
        str(timestamp) + '.log'))
...
def train_model(model, nb_epoch, generators,callbacks=[]):
    ...
    fig.savefig('model_Dataset_Grade_2.png')
