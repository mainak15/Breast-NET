...
#data = DataSet()
train_data_dir = 'Dataset0/train'
valid_data_dir = 'Dataset0/val'
batch_size=8
checkpointer = ModelCheckpoint(
    filepath=os.path.join('Data', 'checkpoints', 'model_BreaKHis_v22.hdf5'),
    verbose=1,
    save_best_only=True)
...
timestamp = time.time()
csv_logger = CSVLogger(os.path.join('Data', 'logs', 'model_BreaKHis_v22' + '-' + 'training-' + \
        str(timestamp) + '.log'))
...
def srima():
    ...
    x1 = Dropout(0.4)(x1)
    x1 = Dense(8, activation='softmax', name='output')(x1)
...
def train_model(model, nb_epoch, generators,callbacks=[]):
    ...
    fig.savefig('model_BreaKHis_v22.png')
